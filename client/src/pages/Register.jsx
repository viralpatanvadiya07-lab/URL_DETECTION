...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Registration failed. Check your DB connection.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      <motion.div 
        ...
        style={{ padding: '3rem', width: '100%', maxWidth: '500px', position: 'relative', overflow: 'hidden' }}
      >
        {/* Decorative background element */}
        <div style={{ position: 'absolute', top: '-50px', right: '-50px', width: '150px', height: '150px', background: 'var(--primary)', borderRadius: '50%', filter: 'blur(80px)', opacity: 0.2 }}></div>

        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            <div style={{ display: 'inline-flex', padding: '1rem', background: 'rgba(14, 165, 233, 0.1)', borderRadius: '1rem', marginBottom: '1rem' }}>
                <ShieldCheck size={32} color="var(--primary)" />
            </div>
            <h2 style={{ fontSize: '2.2rem', fontWeight: '800', marginBottom: '0.5rem' }}>Create Account</h2>
            <p style={{ color: 'var(--text-dim)' }}>Join the community of safe browsing</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: '500', color: 'var(--text-dim)' }}>Full Name</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: '500', color: 'var(--text-dim)' }}>Confirm</label>
                <div style={{ position: 'relative' }}>
                <Lock style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-dim)' }} size={18} />
                <input
        ...
        <div style={{ marginTop: '2rem', textAlign: 'center', fontSize: '0.95rem', color: 'var(--text-dim)' }}>
